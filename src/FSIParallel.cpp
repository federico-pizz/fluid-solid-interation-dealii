...
FSI::setup()
{
  ...
  // Initialize the DoF handler.
  {
    ...
    pcout << "   Number of active cells: " << mesh.n_active_cells()
          << std::endl
  ...
  std::vector<unsigned int> block_sizes(3);
    ...
    block_sizes[1] = n_p;
    block_sizes[2] = n_d;
  // FIX 1: Explicitly create the missing 8th argument (a face evaluation function) 
  // that deal.II 9.5.1 demands. Returning 'true' preserves the default behavior.
  std::function<bool(const DoFHandler<dim>::active_cell_iterator &, unsigned int)> face_eval = 
      [](const DoFHandler<dim>::active_cell_iterator &, unsigned int) { return true; };
        ...
        for (const auto f : cell->face_indices())
        {
          ...
          if (cell->neighbor(f)->has_children())
          {
            for (unsigned int subface = 0; subface < cell->face(f)->n_children(); ++subface)
            {
              ...
              if (cell_is_in_fluid_domain(child)) // Connection: Solid(Coarse) <-> Fluid(Fine)
              {
                 neighbor_dofs.resize(child->get_fe().n_dofs_per_cell());
                 child->get_dof_indices(neighbor_dofs);
                 constraints.add_entries_local_to_global(local_dofs, neighbor_dofs, sparsity);
              }
            }
          }
      ...
      if (cell_is_in_fluid_domain(cell))
        {
          ...
          for (unsigned int i = 0; i < pressure_local_indices.size(); ++i)
            ...
            {
                pressure_submatrix(i, j) = local_pressure_mass_matrix(pressure_local_indices[i], 
                ...
                */
                if ((cell->neighbor(f)->level() == cell->level()) &&
                    ...
                    cell_is_in_fluid_domain(cell->neighbor(f)))
                    {
                      ...
                      neighbor_dof_indices.resize(cell->neighbor(f)->get_fe().n_dofs_per_cell());
                      ...
                      for (unsigned int subface = 0;
                           ...
                           ++subface)
                        {
                            ...
                            cell->neighbor_child_on_subface(f, subface)))
                          {
                            ...
                            auto neighbor_child =
                                cell->neighbor_child_on_subface(f, subface);  
                            neighbor_dof_indices.resize(neighbor_child->get_fe().n_dofs_per_cell());
...
void FSI::refine_mesh()
{
  ...
  // For the parallel implementation we use the parallel versopm of GridRefinement
  parallel::distributed::GridRefinement::refine_and_coarsen_fixed_number(mesh,
