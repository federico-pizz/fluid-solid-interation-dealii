...
void FSI::setup() {
  ...
  // Initialize the finite element space.
  {
    ...
    std::cout << "  Pressure degree:           = " << fe_scalar_pressure.degree
              << std::endl;
    std::cout << "  Displacement degree:           = "
              << fe_scalar_displacement.degree << std::endl;
    std::cout << "  DoFs per cell              = "
    ...
    std::cout << "  Stokes quadrature points per cell = "
              << stokes_quadrature->size() << std::endl;
    std::cout << "  Elasticity quadrature points per cell = "
    ...
    std::cout << "  Quadrature points per face = "
              << common_face_quadrature->size() << std::endl;
  }
  ...
  // Initialize the DoF handler.
  {
    ...
    DoFTools::make_flux_sparsity_pattern(dof_handler, dsp, coupling,
                                         face_coupling);
    ...
    DoFTools::make_sparsity_pattern(dof_handler, coupling, dsp_mass);
    ...
    std::cout << "  Initializing the solution vectors" << std::endl;
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()) {
    ...
    if (cell_is_in_fluid_domain(cell)) {
      constraints.distribute_local_to_global(local_pressure_mass_matrix,
                                             local_dof_indices, pressure_mass);
    }
    ...
    if (cell_is_in_solid_domain(cell)) {
      for (const auto f : cell->face_indices()) {
        if (cell->face(f)->at_boundary() == false) {
          ...
          /**
           * Case 2: The neighbor has further chigit config --global core.editor
           * "vim"ldren. We loop over all children of the neighbor to check if
           * they are part of the fluid subdomain.
           */
...
void FSI::solve() {
  ...
  DoFTools::extract_constant_modes(
      dof_handler, fe_collection.component_mask(displacements), constant_modes);
...
void FSI::run() {
  ...
  std::string csv_filename =
      "timing_results_serial_" + std::to_string(dim) + "D.csv";
  {
    std::ofstream out_file(csv_filename);
    out_file << "cycle,mpi_size,omp_threads,active_cells,total_dofs,setup_time,"
  ...
  for (unsigned int refinement_cycle = 0; refinement_cycle < 10 - 2 * dim;
    ...
    double setup_time = setup_timer.wall_time();
    double assemble_time = assemble_timer.wall_time();
    double solve_time = solve_timer.wall_time();
    double cycle_time = cycle_timer.wall_time();

    {
      std::ofstream out_file(csv_filename, std::ios::app);
      out_file << refinement_cycle << "," << 1 << "," << 1 << ","
  ...
  total_timer.stop();
  double total_wall_time = total_timer.wall_time();
  {
    std::ofstream out_file(csv_filename, std::ios::app);
